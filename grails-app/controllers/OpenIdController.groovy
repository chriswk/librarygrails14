...
import com.chriswk.library.domain.Collector
import com.chriswk.library.domain.Role
import com.chriswk.library.domain.CollectorRole
...
class OpenIdController {
	...
	private boolean createNewAccount(String username, String password, String openId) {
		Collector.withTransaction { status ->
			def config = SpringSecurityUtils.securityConfig

			password = springSecurityService.encodePassword(password)
			def user = new Collector(username: username, password: password, enabled: true)
			...
			for (roleName in config.openid.registration.roleNames) {
				CollectorRole.create user, Role.findByAuthority(roleName)
			}
		}
		true
	}
	...
	private void registerAccountOpenId(String username, String password, String openId) {
		...
		Collector.withTransaction { status ->
			def user = Collector.findByUsername(username)
			user.addToOpenIds(url: openId)
			if (!user.validate()) {
...
class OpenIdRegisterCommand {
	...
	static constraints = {
		username blank: false, validator: { String username, command ->
			Collector.withNewSession { session ->
				if (username && Collector.countByUsername(username)) {
